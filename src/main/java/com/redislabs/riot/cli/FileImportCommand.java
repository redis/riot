...
@Slf4j
@CommandLine.Command(name = "file", description = "Import file")
public class FileImportCommand extends AbstractImportCommand<Map<String, String>> {
    ...
    protected ItemProcessor<Map<String, String>, Object> processor() {
        ...
        switch (fileType) {
            ...
            case JSON:
            case XML:
                return getParentCommand().processor(Collections.singletonList(MapFlattener.builder().build()));
        }
        throw new IllegalArgumentException("Unknown file type: " + fileType);
    }
    ...
    private ItemProcessor stringMapProcessor() {
        ...
        if (!getParentCommand().getSpel().isEmpty()) {
            processors.add(getParentCommand().spelProcessor());
            processors.add(ObjectMapToStringMapProcessor.builder().build());
        }
        return getParentCommand().processor(processors);
    }
}
