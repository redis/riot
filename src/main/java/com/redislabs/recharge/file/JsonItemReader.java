	...
	protected Map<String, Object> doRead() throws Exception {
		...
		try {
			...
			while (true) {
				...
				switch (nextEvent) {
				/*
				 * If we hit the end of an array/object, there will always be an excess comma
				 * from the other switch statement. Remove it before closing the array/object.
				 */
				case END_ARRAY:
					if (0 == nestingLevel) {
						noInput = true;
				...
				case END_OBJECT:
					...
					json = json.append('}');
					/*
					 * we need to track the nesting level. only the end of the root object should
				...
				/*
				 * For any value inside an object/array, we need to add a comma. This will
				 * create a problem with the last element, but we can remove the extra comma at
				 * that time.
				 */
				switch (nextEvent) {
				case END_ARRAY:
