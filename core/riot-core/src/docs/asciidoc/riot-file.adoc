...
{project-title} is a file import/export tool for Redis built on top of https://github.com/redis-developer/lettucemod/[LettuceMod].
...
The `import` command reads from files and writes to Redis.
...
The `export` command reads data from a Redis database and writes it to a JSON or XML file, potentially gzip-compressed.
