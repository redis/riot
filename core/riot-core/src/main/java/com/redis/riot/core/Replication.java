...
public class Replication extends AbstractExport {
	...
	protected RiotContext createContext() {
		ReplicationContext context = new ReplicationContext(super.createContext(), redisContext(targetRedisOptions));
	...
	private FaultTolerantStepBuilder<KeyValue<byte[]>, KeyValue<byte[]>> scanStep(ReplicationContext context)
			throws Exception {
		return step(context, STEP_SCAN, reader("scan-reader", context.getRedisContext()));
	}

	private FaultTolerantStepBuilder<KeyValue<byte[]>, KeyValue<byte[]>> step(ReplicationContext context, String name,
			RedisItemReader<byte[], byte[], KeyValue<byte[]>> reader) throws Exception {
		...
		ItemProcessor<KeyValue<byte[]>, KeyValue<byte[]>> processor = new FunctionItemProcessor<>(
				processor(ByteArrayCodec.INSTANCE, context));
	...
	private FaultTolerantStepBuilder<KeyValue<byte[]>, KeyValue<byte[]>> liveStep(ReplicationContext context)
		...
		RedisItemReader<byte[], byte[], KeyValue<byte[]>> reader = reader("live-reader", context.getRedisContext());
	...
	private RedisItemReader<byte[], byte[], KeyValue<byte[]>> reader(String name, RedisContext context)
			throws Exception {
		AbstractKeyValueItemReader<byte[], byte[]> reader = reader(context.getClient());
		configureReader(name(name), reader, context);
		return reader;
	}
	...
	private TaskletStep compareStep(ReplicationContext context) throws Exception {
		...
		FaultTolerantStepBuilder<KeyComparison, KeyComparison> step = step(STEP_COMPARE, reader, null, writer);
		if (showDiffs) {
	...
	private KeyComparisonItemReader comparisonReader(ReplicationContext context) throws Exception {
		AbstractKeyValueItemReader<String, String> sourceReader = comparisonKeyValueReader(
				context.getRedisContext().getClient());
		configureReader("source-comparison-reader", sourceReader, context.getRedisContext());
		AbstractKeyValueItemReader<String, String> targetReader = comparisonKeyValueReader(
				context.getTargetRedisContext().getClient());
		...
		configureReader("comparison-reader", comparisonReader, context.getRedisContext());
