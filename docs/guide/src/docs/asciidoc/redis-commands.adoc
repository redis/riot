...
....
[green]#{connector}# import ... [yellow]*hset* --keyspace blah --keys id
....

.Import into hashes *and* set TTL on the key
[subs="attributes,+quotes"]
....
[green]#{connector}# import ... [yellow]*hset* --keyspace blah --keys id [yellow]*expire* --keyspace blah --keys id
....

.Import into hashes *and* set TTL *and* add to a set named `myset`
[subs="attributes,+quotes"]
....
[green]#{connector}# import ... [yellow]*hset* --keyspace blah --keys id [yellow]*expire* --keyspace blah --keys id [yellow]*sadd* --keyspace myset --members id
....
...
[subs="attributes,+quotes"]
....
[green]#{connector}# import ... [yellow]*hset* [.line-through]#[red]*-h myredis.com -p 6380*#
....
====
