...
....
[green]#{connector}# import --process epoch=[yellow]#"#date.parse(mydate).getTime()"# location=[yellow]#"#geo(lon,lat)"# id=[yellow]#"#index"# name=[yellow]#"#redis.hget('person1','lastName')"# ...
....
...
[subs="attributes,+quotes"]
....
[green]#{connector}# import --regex name=[yellow]#"(?<first>\w+)\/(?<last>\w+)"# ...
....
